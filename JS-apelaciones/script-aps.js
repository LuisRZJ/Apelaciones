...
emailjs.init('YdNF4w99dWvDnL1ij');
...
document.getElementById('appealForm').addEventListener('submit', function(e) {
    ...
    const formData = {
        ...
        situationDetails: document.getElementById('situationDetails').value,
        appealReason: document.getElementById('appealReason').value,
        brokenRules: Array.from(document.getElementById('brokenRules').selectedOptions)
                        .map(option => option.textContent).join(', '),
        acceptRules: document.getElementById('acceptRules').checked ? 'Sí' : 'No',
        acceptFinal: document.getElementById('acceptFinal').checked ? 'Sí' : 'No'
    };

    // Enviar correo usando tu Service ID y Template ID
    emailjs.send('service_8g2txjv', 'template_u6ghg63', formData)
        .then(() => {
            this.classList.add('hidden');
            document.getElementById('confirmationMessage').classList.remove('hidden');
        })
        .catch((error) => {
            alert('Error al enviar la apelación: ' + error.text);
